...
[ExpressDB](https://github.com/MineralsCloud/ExpressDB.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("ExpressDB")
   ```

3. Run

   ```julia-repl
   julia> using ExpressDB
   ```
...
version of ExpressDB, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/ExpressDB.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/ExpressDB.jl)
for new releases.
Once we release a new version, you can update ExpressDB by typing

```@repl
using Pkg
Pkg.update("ExpressDB")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall ExpressDB
