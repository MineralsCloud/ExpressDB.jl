...
!!! info
    The style guide is always a work in progress, and not all ExpressDB code
    follows the rules. When modifying ExpressDB, please fix the style violations
...
`ExpressDB` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/ExpressDB.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `ExpressDB` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `ExpressDB` have
